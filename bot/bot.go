...
import (
	...
	"github.com/rmerry/tncbot/bot/actions"
...
func (b *Bot) Connect() error {
	...
	for {
		...
		fmt.Print(strings.ToUpper(msg.Value))
		if linkDescription, ok := actions.Execute(msg.Value); ok {
			b.ircclient.SendMessage(&ircclient.IRCMessage{Value: linkDescription})
		} else if strings.Contains(strings.ToUpper(msg.Value), "JOHN") {
			b.ircclient.SendMessage(&ircclient.IRCMessage{Value: "Where's John?"})
		} else if strings.ToUpper(strings.TrimRight(msg.Value, "\r\n")) == "CHUCK" {
			b.ircclient.SendMessage(&ircclient.IRCMessage{Value: chuck[rand.Intn(len(chuck))]})
		}
	}
